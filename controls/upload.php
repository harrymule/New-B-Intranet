...
class Upload {
	...
	function delete_file($file_name){
		...
		$file = UPLOAD_FOLDER.'sq_'.$file_name;
		$check = file_exists($file) ? true : false;
		$size += ($check) ? filesize($file) : 0 ;
		($check) ? unlink($file) : NULL;
		
		$file = UPLOAD_FOLDER.'1_'.$file_name;
		$check = file_exists($file) ? true : false;
		$size += ($check) ? filesize($file) : 0 ;
		($check) ? unlink($file) : NULL;
		
		$file = UPLOAD_FOLDER.'2_'.$file_name;
		$check = file_exists($file) ? true : false;
		$size += ($check) ? filesize($file) : 0 ;
		($check) ? unlink($file) : NULL;
		
		$file = UPLOAD_FOLDER.'3_'.$file_name;
		$check = file_exists($file) ? true : false;
		$size += ($check) ? filesize($file) : 0 ;
		($check) ? unlink($file) : NULL;
		
		$file = UPLOAD_FOLDER.'4_'.$file_name;
		$check = file_exists($file) ? true : false;
		$size += ($check) ? filesize($file) : 0 ;
		($check) ? unlink($file) : NULL;
	...
	function upload_image($fileIndex){
		...
		if(!is_dir($dir)) { mkdir($dir);}
			...
			$this->crop_imge($file['file'],'bunner_'.$file['file'],1000,500);
			return $file['file'];
		}
	...
	function crop_imge($filename,$new_name,$width,$height){
		...
		for($j=1; $j<5; $j++){
			$thumb = imagecreatetruecolor($nw*($j/5),$nh*($j/5));
			imagecopyresampled($thumb,$src,0,0,0,0,$nw*($j/5),$nh*($j/5),$w,$h);
			$save = UPLOAD_FOLDER.$j.'_'.$new_name;
			imagejpeg($thumb,$save,100);
			}	
		}
